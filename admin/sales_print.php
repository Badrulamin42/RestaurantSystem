<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, sales.id AS salesid FROM sales LEFT JOIN users ON users.id=sales.user_id WHERE sales_date BETWEEN '$from' AND '$to' AND pay ='2' ORDER BY sales_date ASC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
	...
	if(isset($_POST['print'])){
	    ...
	    $content = '';  
	    $content .= '
	      	<h2 align="center">Minz IT Solutions</h2>
