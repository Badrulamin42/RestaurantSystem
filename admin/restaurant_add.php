	...
	if(isset($_POST['add'])){
		...
		if($row['restaurant'] > 0){
			...
			try{
				...
				$stmt->execute(['email'=>$email, 'password'=>'Default', 'firstname'=>$firstname, 'lastname'=>$lastname, 'address'=>$address, 'contact'=>$contact, 'photo'=>$filename, 'status'=>1, 'created_on'=>$now, 'type'=>4]);
